/*
 *
 * Helper: `collectReferralDetailsFromApis`.
 *
 */
...
const getCleanText = (raw = "") =>
  raw
    .replace(/\\["nrt]/g, " ") // remove escaped quotes and whitespace
    .replace(/\s+/g, " ") // collapse spaces
    .trim();

const collectReferralDetailsFromApis = async (page, referralId) => {
  const apiTargets = Object.entries(apiTargetsMap);
  let expectedResponses = [];

  try {
    expectedResponses = apiTargets.map(([key, endpoint]) =>
      page
        .waitForResponse(
          (res) =>
    ...
    console.log(
      "❌ An error occurred collecting patient data from apis",
      error.message
    );
  }
  ...
  const results = await Promise.all(expectedResponses);
  const entries = Object.fromEntries(results);

  const getSafe = (key) => {
    const res = entries[key];
    return res?.statusCode === "Success" ? res.data : null;
  };
  ...
  if (icdsData?.data?.length) {
    finalResult.icds = icdsData.data.map((icd) => {
...
export default collectReferralDetailsFromApis;
