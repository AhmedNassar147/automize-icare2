...
const getExtraTimeBasedLogs = async ({
    ...
    if (isLastTodayDiffNegative) {
      ...
      if (negativeDiffCount >= 2) {
        if (doesSystemReducingWait) {
          ...
          if (shouldDecreaseInitialWait) {
            ...
            extraWait = Math.max(-5, extraWait); // tried at day 27
            extraBotMessages.push(
      ...
      const value = isFirstCaseToday
        ...
        : // : doesSystemReducingWait
          ...
          ? Math.min(-6, maxNewWait) // untill day 26
          : maxNewWait;
      extraWait += value;
      const tag =
        doesSystemReducingWait && !isFirstCaseToday
  ...
  if (!isCurrentDiffNegative) {
    ...
    if (!doesSystemReducingWait && IS_NORMAL_REDUCTION_ACTIVE) {
      if (
        !isFirstCaseToday &&
        value < 4 &&
        !isHotCluster
...
// const fullLog = await readLogsAsArray();
// const currentDayKey = getLocalDayKey(1785312759000);
// const todayCases = getTodayCases(fullLog, currentDayKey);
// const data = todayCases.slice(-8);

// const positiveDiffCount = getDiffCountBeforeCurrent(false, data, 0);

// console.log("positiveDiffCount", positiveDiffCount);
