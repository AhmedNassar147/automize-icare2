/*
 *
 * Helper: `collectReferralInfoFromApis`.
 *
 */
const apiTargetsMap = {
  ...
  try {
    expectedResponses = apiTargets.map(([key, endpoint]) =>
      page
        ...
        .then((res) => {
          console.log(`✅ Got patient=${referralId} ${key} response`);
          return [key, res.json()];
        })
        .catch(() => [key, null])
    );
    ...
    console.log(
      "❌ An error occurred collecting patient info from apis",
      error.message
    );
  }
...
export default collectReferralInfoFromApis;
